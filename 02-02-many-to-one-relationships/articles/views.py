...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # 현재 유저가 누구인지 찾을 필요가 없다. 왜? 이미 로그인되어 있으니까.
            # 즉, 이미 요청 객체에 어떤 사람인지가 들어있기 때문에 별도로 조회할 필요가 없다.
...
def update(request, pk):
    article = Article.objects.get(pk=pk)
    # 수정을 시도하는 사용자와 게시글의 작성자가 같은지 확인
    if request.user == article.user:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                # article.save()
                # 위처럼 하지 않는 이유? 이미 instance 로 article 을 받았기 때문.
                # 즉, 이미 유저가 누구인지 알고 있다.
                return redirect('articles:detail', article.pk)
        else:
